...
rf_baseline <- randomForest(
  Conversion ~ .,
  data = train_df,
  mtry = floor(sqrt(ncol(X_train))), #default value for mtry parameter for classification
  nodesize = 1, #default value of nodesize parameter
  ntree = 200, #default value of ntree parameter
  weights = obs_weight,
  importance = TRUE
)
...
#================================================
#TUNED-FINAL RANDOM FOREST MODEL
#================================================

# ============================================================
# HYPERPARAMETER TUNING WITH CROSS-VALIDATION
# Hyperparameters Tuned: mtry, nodesize, ntree 
# ============================================================
...
#defining mtry hyperparameter values for optimisation
mtry_values <- c(
  floor(sqrt(n_features)),           #default value
  floor(n_features / 2) #1/2 of features
)

#defining nodesize hyperparameter values for optimisation
...
# 5-fold CV implemented manually
set.seed(123)
n_folds <- 5 #number of folds (5-fold cross validation)
folds <- createFolds(train_df$Conversion, k = n_folds, list = TRUE)


#initialization of parameters and auc value
best_auc<-0
best_mtry<-NULL
best_nodesize<-NULL
best_ntree<-NULL
...
for (i in 1:nrow(param_grid)){
  ...
  #comparing and finding the best values for each parameter based on AUC value
...
#BEST/OPTIMIZED HYPERPARAMETERS results
...
#training final model with the best/optimal combination of hyperparameters
set.seed(123)
...
# Extract all the evaluation metrics from the confusion matrix
...
# display the results table of the random forest model
View(results_rf)
...
plot(pr_obj,
     col="blue",
     lwd=2,
     main=paste("PR Curve (Precision Recall Curve (PR-AUC = ",round(pr_obj$auc.integral,4),")")
)
